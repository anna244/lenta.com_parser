...
class LentaComParser:
    ...
    def __init__(self, *, outfile_name: str, store_ids: list[str]):
        ...
        self._client = None
        # allow for 100 concurrent entries within a 30 seconds window
        self._limiter = AsyncLimiter(max_rate=50, time_period=30)
    ...
    def client(self):
        ...
        if self._client is None or self._client._state == httpx._client.ClientState.CLOSED:

            self._client = httpx.AsyncClient(
                timeout=30,
                limits=httpx.Limits(
                    max_keepalive_connections=self.API_SKUS_BATCH_SIZE,
                    max_connections=self.API_SKUS_BATCH_SIZE * 2
                ),
                headers={
    ...
    async def get_list_of_goods(
        ...
        while current_try < retries:
            try:
                ...
                current_try += 1
                logger.error(
                    f'Network error while getting list of goods for store_id: {store_id}, data: {data}, error: {e}. '
